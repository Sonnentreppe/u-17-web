...
if($input->get('do')=='add'){
  ...
  if(empty($title)||empty($author)||empty($content)){
    ...
    if($pid>0){
      $intime=time();
      $strTpl="update page set title='%s',content='%s',author='%s',uptime='%d' where pid='%d'";
      $sql= sprintf($strTpl, $title, $author, $content, $uptime,$pid);
    }else{
      $intime=time();
      $strTpl="insert into page (`title`, `author`, `content`, `intime`, `uptime`) values ('%s', '%s', '%s', '%d', '%d')";
